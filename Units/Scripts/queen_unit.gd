...
var arrow: PackedScene = preload("res://Towers/Scenes/archer_arrow.tscn")
...
func attack():
	...
	new_arrow.direction = (tower.global_position - $ShootPosition.global_position).normalized()
	new_arrow.rotation = new_arrow.direction.angle() + PI / 2
	new_arrow.damage = damage
	new_arrow.user = "Player"
...
func _on_hit_box_area_entered(area: Area2D) -> void:
	if area.is_in_group("Projectiles") and area.get_parent().user == "Enemy":
...
func _on_shoot_area_area_exited(area: Area2D) -> void:
	if area.is_in_group("Tower"):
		at_tower = false
		tower = null
